...
kernel void compute_tree(
                       ...
                       //
                       , device const uint* trNds
                       , device const uint* trV0
                       , device const uint* trV1
                       , device const uint* trV2
                       , device const uint* trV3
                       //
                       , device uint* flNds
                       , device uint* flV0
                       , device uint* flV1
...
{
    uint iNodeSrc   = params[0] /*iNodeOffset*/ + index;
    uint iNode      = iNodeSrc;
    uint iDeepLvl   = 0u;
    uint iParent    = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_iParent ) / 4u ];
    uint pck        = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_pck ) / 4u ];
    ...
    float rad       = radians( as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_deg ) / 4u ] ) );
    float radSin    = sin( rad );
    float radCos    = cos( rad );
    float sx        = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_sX ) / 4u ] );
    float sy        = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_sY ) / 4u ] );
    uint c8         = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_c ) / 4u ];
    ...
    uint vsFirstIdx = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_vs_iFirst ) / 4u ];
    uint vsCount    = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_vs_count ) / 4u ];
    float mDet;
    float2x3 mP, mC, m;
    m[0][0] = radCos * sx;
    m[0][1] = -radSin * sy;
    m[0][2] = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_x ) / 4u ] );
    m[1][0] = radSin * sx;
    m[1][1] = radCos * sy;
    m[1][2] = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_y ) / 4u ] );
    while(iNode != iParent){
        iNode = iParent;
        iParent = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_iParent ) / 4u ];
        iDeepLvl++;
        pck = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_pck ) / 4u ];
        ...
        rad = radians( as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_deg )  / 4u ] ) );
        radSin = sin( rad );
        radCos = cos( rad );
        sx = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_sX ) / 4u ] );
        sy = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_sY ) / 4u ] );
        mC = m;
        mP[0][0] = radCos * sx;
        mP[0][1] = -radSin * sy;
        mP[0][2] = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_x ) / 4u ] );
        mP[1][0] = radSin * sx;
        mP[1][1] = radCos * sy;
        mP[1][2] = as_type<float>( trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_t_y ) / 4u ] );
        ...
        c8 = trNds[ ( iNode * NBScnTreeNode_SZ + NBScnTreeNode_IDX_c ) / 4u ];
        ...
        switch (vsType) {
            case 0:
                for (vIdx = vsFirstIdx; vIdx < vIdxAfterLast; vIdx++) {
                    vX = as_type<float>( trV0[ ( vIdx * NBScnVertex_SZ + NBScnVertex_IDX_x ) / 4u ] );
                    vY = as_type<float>( trV0[ ( vIdx * NBScnVertex_SZ + NBScnVertex_IDX_y ) / 4u ] );
                    //
                    vXP = (m[0][0] * vX) + (m[0][1] * vY) + m[0][2];
                    vYP = (m[1][0] * vX) + (m[1][1] * vY) + m[1][2];
                    //
                    c8 = trV0[ ( vIdx * NBScnVertex_SZ + NBScnVertex_IDX_color ) / 4u ];
                    ...
                    //
                    flV0[ ( vIdx * NBScnVertexF_SZ + NBScnVertexF_IDX_x) / 4u ] = as_type<uint>(vXP);
                    flV0[ ( vIdx * NBScnVertexF_SZ + NBScnVertexF_IDX_y) / 4u ] = as_type<uint>(vYP);
                    flV0[ ( vIdx * NBScnVertexF_SZ + NBScnVertexF_IDX_color_r) / 4u ] = as_type<uint>(vCR);
                    flV0[ ( vIdx * NBScnVertexF_SZ + NBScnVertexF_IDX_color_g) / 4u ] = as_type<uint>(vCG);
                    flV0[ ( vIdx * NBScnVertexF_SZ + NBScnVertexF_IDX_color_b) / 4u ] = as_type<uint>(vCB);
                    flV0[ ( vIdx * NBScnVertexF_SZ + NBScnVertexF_IDX_color_a) / 4u ] = as_type<uint>(vCA);
                }
                break;
            case 1:
                for (vIdx = vsFirstIdx; vIdx < vIdxAfterLast; vIdx++) {
                    vX = as_type<float>( trV1[ ( vIdx * NBScnVertexTex_SZ + NBScnVertexTex_IDX_x ) / 4u ] );
                    vY = as_type<float>( trV1[ ( vIdx * NBScnVertexTex_SZ + NBScnVertexTex_IDX_y ) / 4u ] );
                    //
                    vXP = (m[0][0] * (vX)) + (m[0][1] * (vY)) + m[0][2];
                    vYP = (m[1][0] * (vX)) + (m[1][1] * (vY)) + m[1][2];
                    //
                    c8 = trV1[ ( vIdx * NBScnVertexTex_SZ + NBScnVertexTex_IDX_color ) / 4u ];
                    vCR = r * (float(NBColor8_getR(c8)) / 255.f);
                    vCG = g * (float(NBColor8_getG(c8)) / 255.f);
                    vCB = b * (float(NBColor8_getB(c8)) / 255.f);
                    vCA = a * (float(NBColor8_getA(c8)) / 255.f);
                    //
                    t1x = as_type<float>( trV1[ ( vIdx * NBScnVertexTex_SZ + NBScnVertexTex_IDX_tex_x ) / 4u ] );
                    t1y = as_type<float>( trV1[ ( vIdx * NBScnVertexTex_SZ + NBScnVertexTex_IDX_tex_y ) / 4u ] );
                    //
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_x) / 4u ] = as_type<uint>(vXP);
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_y) / 4u ] = as_type<uint>(vYP);
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_color_r) / 4u ] = as_type<uint>(vCR);
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_color_g) / 4u ] = as_type<uint>(vCG);
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_color_b) / 4u ] = as_type<uint>(vCB);
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_color_a) / 4u ] = as_type<uint>(vCA);
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_tex_x) / 4u ] = as_type<uint>(t1x);
                    flV1[ ( vIdx * NBScnVertexTexF_SZ + NBScnVertexTexF_IDX_tex_y) / 4u ] = as_type<uint>(t1y);
                }
                break;
            case 2:
                for (vIdx = vsFirstIdx; vIdx < vIdxAfterLast; vIdx++) {
                    vX = as_type<float>( trV2[ ( vIdx * NBScnVertexTex2_SZ + NBScnVertexTex2_IDX_x ) / 4u ] );
                    vY = as_type<float>( trV2[ ( vIdx * NBScnVertexTex2_SZ + NBScnVertexTex2_IDX_y ) / 4u ] );
                    //
                    vXP = (m[0][0] * (vX)) + (m[0][1] * (vY)) + m[0][2];
                    vYP = (m[1][0] * (vX)) + (m[1][1] * (vY)) + m[1][2];
                    //
                    c8 = trV2[ ( vIdx * NBScnVertexTex2_SZ + NBScnVertexTex2_IDX_color ) / 4u ];
                    vCR = r * (float(NBColor8_getR(c8)) / 255.f);
                    vCG = g * (float(NBColor8_getG(c8)) / 255.f);
                    vCB = b * (float(NBColor8_getB(c8)) / 255.f);
                    vCA = a * (float(NBColor8_getA(c8)) / 255.f);
                    //
                    t1x = as_type<float>( trV2[ (vIdx * NBScnVertexTex2_SZ + NBScnVertexTex2_IDX_tex_x ) / 4u ] );
                    t1y = as_type<float>( trV2[ (vIdx * NBScnVertexTex2_SZ + NBScnVertexTex2_IDX_tex_y ) / 4u ] );
                    t2x = as_type<float>( trV2[ (vIdx * NBScnVertexTex2_SZ + NBScnVertexTex2_IDX_tex2_x ) / 4u ] );
                    t2y = as_type<float>( trV2[ (vIdx * NBScnVertexTex2_SZ + NBScnVertexTex2_IDX_tex2_y ) / 4u ] );
                    //
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_x) / 4u ] = as_type<uint>(vXP);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_y) / 4u ] = as_type<uint>(vYP);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_color_r) / 4u ] = as_type<uint>(vCR);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_color_g) / 4u ] = as_type<uint>(vCG);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_color_b) / 4u ] = as_type<uint>(vCB);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_color_a) / 4u ] = as_type<uint>(vCA);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_tex_x) / 4u ] = as_type<uint>(t1x);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_tex_y) / 4u ] = as_type<uint>(t1y);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_tex2_x) / 4u ] = as_type<uint>(t2x);
                    flV2[ ( vIdx * NBScnVertexTex2F_SZ + NBScnVertexTex2F_IDX_tex2_y) / 4u ] = as_type<uint>(t2y);
                }
                break;
            case 3:
                for (vIdx = vsFirstIdx; vIdx < vIdxAfterLast; vIdx++) {
                    vX = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_x ) / 4u ] );
                    vY = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_y ) / 4u ] );
                    //
                    vXP = (m[0][0] * (vX)) + (m[0][1] * (vY)) + m[0][2];
                    vYP = (m[1][0] * (vX)) + (m[1][1] * (vY)) + m[1][2];
                    //
                    c8 = trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_color ) / 4u ];
                    vCR = r * (float(NBColor8_getR(c8)) / 255.f);
                    vCG = g * (float(NBColor8_getG(c8)) / 255.f);
                    vCB = b * (float(NBColor8_getB(c8)) / 255.f);
                    vCA = a * (float(NBColor8_getA(c8)) / 255.f);
                    //
                    t1x = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_tex_x ) / 4u ] );
                    t1y = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_tex_y ) / 4u ] );
                    t2x = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_tex2_x ) / 4u ] );
                    t2y = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_tex2_y ) / 4u ] );
                    t3x = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_tex3_x ) / 4u ] );
                    t3y = as_type<float>( trV3[ ( vIdx * NBScnVertexTex3_SZ + NBScnVertexTex3_IDX_tex3_y ) / 4u ] );
                    //
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_x) / 4u ] = as_type<uint>(vXP);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_y) / 4u ] = as_type<uint>(vYP);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_color_r) / 4u ] = as_type<uint>(vCR);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_color_g) / 4u ] = as_type<uint>(vCG);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_color_b) / 4u ] = as_type<uint>(vCB);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_color_a) / 4u ] = as_type<uint>(vCA);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_tex_x) / 4u ] = as_type<uint>(t1x);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_tex_y) / 4u ] = as_type<uint>(t1y);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_tex2_x) / 4u ] = as_type<uint>(t2x);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_tex2_y) / 4u ] = as_type<uint>(t2y);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_tex3_x) / 4u ] = as_type<uint>(t3x);
                    flV3[ ( vIdx * NBScnVertexTex3F_SZ + NBScnVertexTex3F_IDX_tex3_y) / 4u ] = as_type<uint>(t3y);
                }
                break;
            default:
                break;
    ...
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_pck) / 4u ] = pck;
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_m_0_0) / 4u ] = as_type<uint>(m[0][0]);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_m_0_1) / 4u ] = as_type<uint>(m[0][1]);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_m_0_2) / 4u ] = as_type<uint>(m[0][2]);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_m_1_0) / 4u ] = as_type<uint>(m[1][0]);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_m_1_1) / 4u ] = as_type<uint>(m[1][1]);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_m_1_2) / 4u ] = as_type<uint>(m[1][2]);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_c_r) / 4u ] = as_type<uint>(r);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_c_g) / 4u ] = as_type<uint>(g);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_c_b) / 4u ] = as_type<uint>(b);
    flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_c_a) / 4u ] = as_type<uint>(a);
    ...
    if(mDet != 0.0f){
        ...
        flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_mInv_0_0) / 4u ] = as_type<uint>(mC[0][0]);
        flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_mInv_0_1) / 4u ] = as_type<uint>(mC[0][1]);
        flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_mInv_0_2) / 4u ] = as_type<uint>(mC[0][2]);
        flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_mInv_1_0) / 4u ] = as_type<uint>(mC[1][0]);
        flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_mInv_1_1) / 4u ] = as_type<uint>(mC[1][1]);
        flNds[ ( iNodeSrc * NBScnFlatNode_SZ + NBScnFlatNode_IDX_mInv_1_2) / 4u ] = as_type<uint>(mC[1][2]);
    }
}

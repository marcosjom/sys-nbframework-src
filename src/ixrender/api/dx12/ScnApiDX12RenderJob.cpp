...
ScnBOOL ScnApiDX12RenderJob_buildBegin(void* data, ScnGpuBufferRef pBuffPropsScns, ScnGpuBufferRef pBuffPropsMdls){
    ScnBOOL r = ScnFALSE;
    STScnApiDX12RenderJob* obj = (STScnApiDX12RenderJob*)data;
    //
    ...
    if(obj->dev == NULL || obj->dev->dev == nil || obj->dev->cmdQueue == nil){
        SCN_PRINTF_ERROR("ScnApiDX12RenderJob_buildBegin::dev-or-cmdQueue is NULL.\n");
        return ScnFALSE;
    }
    if(ScnGpuBuffer_isNull(pBuffPropsScns) || ScnGpuBuffer_isNull(pBuffPropsMdls)){
        //SCN_PRINTF_ERROR("ScnApiDX12RenderJob_buildBegin::pBuffPropsScns-or-pBuffPropsMdls is NULL.\n");
        return ScnFALSE;
    }
    ...
    if(bPropsScns == NULL || bPropsScns->buff == nil){
        SCN_PRINTF_ERROR("ScnApiDX12RenderJob_buildBegin::bPropsScns is NULL.\n");
        return ScnFALSE;
    } else if(bPropsMdls == NULL || bPropsMdls->buff == nil){
        SCN_PRINTF_ERROR("ScnApiDX12RenderJob_buildBegin::bPropsMdls is NULL.\n");
        return ScnFALSE;
    }
    if(obj->cmdsBuff != nil){
        const MTLCommandBufferStatus status = [obj->cmdsBuff status];
        if(status != MTLCommandBufferStatusNotEnqueued && status != MTLCommandBufferStatusCompleted && status != MTLCommandBufferStatusError){
            ...
            SCN_PRINTF_ERROR("ScnApiDX12RenderJob_buildBegin::[obj->cmdsBuff status] is active.\n");
            return ScnFALSE;
        }
    }
    //begin

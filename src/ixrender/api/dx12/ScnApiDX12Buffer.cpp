...
void ScnApiDX12Buffer_free(void* pObj){
...
#ifdef SCN_ASSERTS_ACTIVATED
ScnBOOL ScnApiDX12Buffer_syncValidate_(STScnApiDX12Buffer* obj, ScnMemElasticRef mem);
#endif

ScnBOOL ScnApiDX12Buffer_sync(void* pObj, ScnMemElasticRef mem, const STScnGpuBufferChanges* changes){
    ScnBOOL r = ScnFALSE;
    STScnApiDX12Buffer* obj = (STScnApiDX12Buffer*)pObj;
    ...
    //sync
    {
        ...
        if(cpuBuffSz != buffLen){
            ...
            if(buff != nil){
                SCN_PRINTF_VERB("ScnApiDX12Buffer_sync::gpu-buff resized from %u to %u bytes.\n", buffLen, cpuBuffSz);
                buffLen = cpuBuffSz;
                [obj->buff release];
                obj->buff = buff;
                buffIsNew = ScnTRUE;
        ...
        if(buffLen < cpuBuffSz){
            SCN_PRINTF_ERROR("ScnApiDX12Buffer_sync::gpuBuff is smaller than cpu-buff.\n");
        } else if(buffIsNew || changes->all){
            //sync all
            const STScnRangeU rngAll = ScnMemElastic_getUsedAddressesRngAligned(mem);
            if(!ScnApiDX12Buffer_syncRanges_(obj->buff, mem, &rngAll, 1)){
                SCN_PRINTF_ERROR("ScnApiDX12Buffer_sync::ScnApiDX12Buffer_syncRanges_ failed.\n");
            } else {
                r = ScnTRUE;
                //validate
                //SCN_ASSERT(ScnApiDX12Buffer_syncValidate_(obj, mem))
            }
        } else {
            //sync ranges only
            if(!ScnApiDX12Buffer_syncRanges_(obj->buff, mem, changes->rngs, changes->rngsUse)){
                SCN_PRINTF_ERROR("ScnApiDX12Buffer_sync::ScnApiDX12Buffer_syncRanges_ failed.\n");
            } else {
                r = ScnTRUE;
                //validate
...
#ifdef SCN_ASSERTS_ACTIVATED
typedef struct ScnApiDX12Buffer_syncValidate_st {
    STScnApiDX12Buffer*    obj;
    STScnRangeU             usedAddressesRng;
    ScnUI32                 leftLimitFnd;
    ScnUI32                 rghtLimitFnd;
} ScnApiDX12Buffer_syncValidate_st;
#endif

#ifdef SCN_ASSERTS_ACTIVATED
ScnBOOL ScnApiDX12Buffer_syncValidate_pushBlockPtrs_(void* data, const ScnUI32 rootIndex, const void* rootAddress, const STScnMemBlockPtr* const ptrs, const ScnUI32 ptrsSz){
    ScnBOOL r = ScnFALSE;
    ScnApiDX12Buffer_syncValidate_st* st = (ScnApiDX12Buffer_syncValidate_st*)data;
...
ScnBOOL ScnApiDX12Buffer_syncValidate_(STScnApiDX12Buffer* obj, ScnMemElasticRef mem){
    ScnBOOL r = ScnTRUE;
    ScnApiDX12Buffer_syncValidate_st st;
    ...
    st.rghtLimitFnd     = 0;
    {
        r = ScnMemElastic_pushPtrs(mem, ScnApiDX12Buffer_syncValidate_pushBlockPtrs_, &st);
    }
